...
| Best Bars for Diabetics | `Sugars (g)` ≤ 5 AND net carbs ≤ 10 AND `Dietary Fiber (g)` ≥ 5 AND `Protein (g)` ≥ 10 AND `score_band` in (A, B) AND ingredients do not contain the maltitol family (see below) | 112 | 9.0% |
...
**Best Bars for Diabetics was recomputed again against the 1,245-bar live database on 2026-08-21** (schema v8, database update only, no formula change) — 112 of 1,245 bars qualify (9.0%).
