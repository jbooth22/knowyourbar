...
def compute_insights(pid, ingr_text_full, lines_by_pid, sa_ids, quality_prot_ids, collagen_ids, vitmin_ids):
    """Compute all insight chips for a bar.
    
    Scoring uses pre-parsed ingredient lines (which correctly exclude trace/contains-less-than
    clauses from the score calculation). Chip flags scan the FULL raw ingredient text so that
    ingredients present even at trace amounts are still flagged — a bar containing sucralose
    at any level should show the Artificial Sweeteners chip.
    """
    ...
    # Full text scan — no truncation at "contains less than"
    full_lower = str(ingr_text_full).lower()
    insights = []
    ...
    # Scan FULL text for all chip flags (includes trace/contains-less-than ingredients)
    if any(kw in full_lower for kw in ARTIFICIAL_SW_KW):
