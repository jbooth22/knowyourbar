...
function openCompareOverlay() {
  ...
  function getBest(key, better) {
    ...
    const count = vals.filter(v => v === best).length;
    return count < bars.length ? best : null;
  }
  ...
  const macroRows = MACRO_ROWS.map(({key, label, unit, better}) => {
    ...
    const cells = bars.map(b => {
      ...
      return `<td class="cmp-cell${isBest ? ' cmp-best' : ''}">${v != null ? v + unit : '&mdash;'}</td>`;
    }).join('');
  ...
  const ingrCells = bars.map(b => {
    ...
    return `<td class="cmp-cell cmp-ingr-cell">${posHTML}${negHTML}${full}</td>`;
  }).join('');
  ...
  const barCards = bars.map(b => {
    ...
    const buy = aff && String(aff).startsWith('http') ? `<a href="${aff}" target="_blank" rel="noopener sponsored" class="cmp-buy-btn">Buy on Amazon</a>` : '';
    const site = b['Website'] ? `<a href="${b['Website']}" target="_blank" rel="noopener" class="cmp-site-btn">Product page</a>` : '';
    return `<div class="cmp-bar-card">
  ...
  overlay.innerHTML = `
    <div class="cmp-overlay-inner">
      ...
      <div class="cmp-scroll">
        <div class="cmp-bar-headers-row" style="grid-template-columns:repeat(${colCount},1fr)">
          ${barCards}
        </div>
        <table class="cmp-table">
          <colgroup><col style="width:120px">${bars.map(() => '<col style="min-width:190px">').join('')}</colgroup>
          <tbody>
...
function restoreCompareFromURL() {
  ...
  if (compareSet.size > 0) {
    updateCompareTray();
    if (compareSet.size >= 2) setTimeout(() => openCompareOverlay(), 300);
  }
}
