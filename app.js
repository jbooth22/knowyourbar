...
function serializeState() {
  ...
  // preserve ?compare= if comparison is active
  if (compareSet.size > 0) params.set('compare', [...compareSet.keys()].join(','));
  ...
  display.forEach(bar => {
    ...
    row.innerHTML = `
      ...
      <td class="col-compare">
        <button class="compare-cb${compareSet.has(barSlug(bar)) ? ' active' : ''}"
          data-slug="${barSlug(bar)}"
          aria-pressed="${compareSet.has(barSlug(bar))}"
          title="${compareSet.has(barSlug(bar)) ? 'Remove from comparison' : 'Add to comparison'}">+</button>
      </td>`;
    ...
    // Attach compare button listener AFTER append so stopPropagation works correctly
    const compareBtn = row.querySelector('.compare-cb');
    if (compareBtn) {
      compareBtn.addEventListener('click', e => {
        e.stopPropagation();
        const b = BARS.find(x => barSlug(x) === compareBtn.dataset.slug);
        if (b) toggleCompare(b, e);
      });
    }
  });
...
function toggleExpand(bar, row) {
    ...
    <div class="expand-content">
      ...
      <div class="expand-columns">
        ...
        <div class="expand-right">
          ...
          <button class="expand-compare-btn${compareSet.has(barSlug(bar)) ? ' active' : ''}"
            data-slug="${barSlug(bar)}">
            ${compareSet.has(barSlug(bar)) ? '✓ In comparison' : '+ Add to comparison'}
          </button>
  ...
  // Attach expand row compare button listener
  const expandCompareBtn = expandRow.querySelector('.expand-compare-btn');
  if (expandCompareBtn) {
    expandCompareBtn.addEventListener('click', e => {
      e.stopPropagation();
      const slug = expandCompareBtn.dataset.slug;
      const b = BARS.find(x => barSlug(x) === slug);
      if (b) {
        toggleCompare(b, e);
        // Update button text immediately
        const inSet = compareSet.has(slug);
        expandCompareBtn.textContent = inSet ? '✓ In comparison' : '+ Add to comparison';
        expandCompareBtn.classList.toggle('active', inSet);
      }
    });
  }
}
...
// ─── Comparison Feature ──────────────────────────────
let compareSet = new Map(); // slug -> bar object, max 4

function toggleCompare(bar, e) {
  e.stopPropagation(); // don't trigger row expand
  const slug = barSlug(bar);
...
function updateAllCompareButtons() {
  document.querySelectorAll('.compare-cb').forEach(cb => {
    ...
    cb.setAttribute('aria-pressed', String(active));
    cb.title = active ? 'Remove from comparison' : 'Add to comparison';
  });
}
...
function updateCompareTray() {
      ...
      <div class="tray-bars">
        ${items.map(b => `
          <div class="tray-bar">
            ...
            <button class="tray-remove" data-slug="${barSlug(b)}" aria-label="Remove">×</button>
          </div>
        `).join('')}
      </div>
      <div class="tray-actions">
        <button class="tray-clear">Clear</button>
        <button class="tray-compare ${items.length < 2 ? 'disabled' : ''}"
          ${items.length < 2 ? 'disabled' : ''}>
          Compare ${items.length >= 2 ? items.length + ' bars' : '(need 2+)'}
        </button>
      </div>
    </div>`;

  // Attach tray button listeners (no inline onclick — safer and cleaner)
  ...
  // Push page content up so footer/last rows aren't hidden behind tray
  requestAnimationFrame(() => {
    const trayH = tray.offsetHeight;
    document.body.style.paddingBottom = trayH + 'px';
  });
}
...
function openCompareOverlay() {
  ...
  // Build shareable URL
  const slugs = [...compareSet.keys()].join(',');
  ...
  // For each macro, find best value to highlight
  ...
  const colHeaders = bars.map(b => {
    const band = b['score_band'];
    const color = BAND_COLORS[band] || '#888';
    const aff = b['Amazon Affiliate'];
    const buyBtn = aff ? `<a href="${aff}" target="_blank" rel="noopener sponsored" class="cmp-buy-btn">Buy on Amazon</a>` : '';
    const website = b['Website'] ? `<a href="${b['Website']}" target="_blank" rel="noopener" class="cmp-site-btn">Product page</a>` : '';
    return `
      <div class="cmp-col-header">
        <div class="cmp-brand">${b['Brand Name']}</div>
        <div class="cmp-flavor">${b['Flavor Name']}</div>
        <div class="cmp-grade-badge" style="background:${color}">${band || '?'}</div>
        <div class="cmp-grade-label">${BAND_LABELS[band] || ''} &middot; ${b['ingredient_score'] ?? '—'}</div>
        <div class="cmp-btns">${website}${buyBtn}</div>
      </div>`;
  }).join('');
  ...
  const macroRows = MACRO_ROWS.map(({ key, label, unit, better }) => {
    ...
    const cells = bars.map(b => {
      const v = b[key];
      const isBest = best != null && v === best && bars.filter(x => x[key] === best).length < bars.length;
      return `<td class="cmp-cell${isBest ? ' cmp-best' : ''}">${v != null ? v + unit : '—'}</td>`;
    }).join('');
  ...
  const chipCells = bars.map(b => {
    ...
    return `<td class="cmp-cell cmp-chips-cell">${chips || '—'}</td>`;
  }).join('');
  ...
  const certRows = CERT_COLS.map(([label, col]) => {
    const cells = bars.map(b => {
      ...
      return `<td class="cmp-cell cmp-cert-cell">${yes ? '<span class="cmp-yes">✓</span>' : '<span class="cmp-no">–</span>'}</td>`;
    }).join('');
    return `<tr><td class="cmp-row-label">${label}</td>${cells}</tr>`;
  }).join('');
  ...
  const ingrCells = bars.map(b => {
    const ingr = b['Ingredients'];
    if (!ingr) return `<td class="cmp-cell cmp-ingr-cell"><em>Not available</em></td>`;
    ...
    const fullHTML = `<div class="cmp-ingr-full">${toTitleCase(ingr)}</div>`;
    return `<td class="cmp-cell cmp-ingr-cell">${posHTML}${negHTML}${fullHTML}</td>`;
  }).join('');
    ...
    <div class="cmp-overlay-inner">
      <div class="cmp-header">
        ...
        <div class="cmp-header-actions">
          <button class="cmp-share-btn" onclick="copyCompareLink()" title="Copy shareable link">⎘ Copy link</button>
          <button class="cmp-close-btn" onclick="closeCompareOverlay()">✕ Close</button>
        </div>
      </div>
      <div class="cmp-scroll">

        <div class="cmp-bar-headers-row">
          ${bars.map(b => {
            const band = b['score_band'];
            const color = BAND_COLORS[band] || '#888';
            const aff = b['Amazon Affiliate'];
            const buyBtn = aff && String(aff).startsWith('http') ? `<a href="${aff}" target="_blank" rel="noopener sponsored" class="cmp-buy-btn">Buy on Amazon</a>` : '';
            const website = b['Website'] ? `<a href="${b['Website']}" target="_blank" rel="noopener" class="cmp-site-btn">Product page</a>` : '';
            return `
              <div class="cmp-bar-card">
                <div class="cmp-brand">${b['Brand Name']}</div>
                <div class="cmp-flavor">${b['Flavor Name']}</div>
        ...
        <table class="cmp-table">
          <colgroup>
            ...
            ${bars.map(() => `<col style="min-width:190px">`).join('')}
          </colgroup>
          <tbody>
  ...
  // Set grid column count for bar header cards
  overlay.querySelector('.cmp-bar-headers-row').style.setProperty('--cmp-cols', bars.length);
  overlay.querySelector('.cmp-bar-headers-row').style.gridTemplateColumns = `repeat(${bars.length}, 1fr)`;
...
function closeCompareOverlay() {
  ...
  // Remove compare param from URL but keep other state
  const url = new URL(window.location.href);
...
function copyCompareLink() {
  ...
  navigator.clipboard.writeText(url.toString()).then(() => {
    const btn = document.querySelector('.cmp-share-btn');
...
function restoreCompareFromURL() {
  ...
  slugs.forEach(slug => {
    ...
    if (bar && compareSet.size < 4) compareSet.set(slug, bar);
  });
  if (compareSet.size > 0) {
    updateCompareTray();
    // Auto-open overlay if 2+ bars in URL
    if (compareSet.size >= 2) {
      setTimeout(() => openCompareOverlay(), 300);
    }
  }
}
