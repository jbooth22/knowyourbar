...
function renderSimilarBars(bar) {
  ...
  const catLabel = comp.tCat !== 'other' ? comp.tCat : 'similar flavor';
  const c1 = simCard(comp.b1, 'Same nutrition',
    'Closest macro match across all brands');
  const c2 = simCard(comp.b2, 'Same taste, better macros',
    `More protein or less sugar in the ${catLabel} category`);
  const c3 = comp.isAlreadyA
    ? simCard(comp.b3, 'Equally clean',
        'Another A-grade bar with a similar nutritional profile')
    : simCard(comp.b3, 'Same nutrition, cleaner',
        'Similar macros with a higher ingredient quality grade');

  if (!c1 && !c2 && !c3) return '';

  return `<div class="similar-section">
    <div class="similar-label">Compare this bar</div>
    <div class="similar-cards">${c1}${c2}${c3}</div>
  </div>`;
}
...
function init() {
  ...
  if (barCountEl) barCountEl.textContent = '1,000+';
...
function readURLParams() {
  ...
  // Rewrite legacy ?band= to ?grade= immediately so Google crawls the canonical URL form
  if (params.has('band') && !params.has('grade')) {
    params.set('grade', params.get('band'));
    params.delete('band');
