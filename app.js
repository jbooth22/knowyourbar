    ...
    row.innerHTML = `
      ...
      <td class="col-compare">
        <button class="compare-cb${compareSet.has(barSlug(bar)) ? ' active' : ''}"
          ...
          title="${compareSet.has(barSlug(bar)) ? 'Remove from comparison' : 'Add to comparison'}"
          onclick="handleCompareClick(this, event)">+</button>
      </td>`;
...
function handleCompareClick(btn, e) {
  e.stopPropagation();
  const slug = btn.dataset.slug;
  const bar = BARS.find(b => barSlug(b) === slug);
  if (bar) toggleCompare(bar, e);
}
      ...
      <div class="tray-bars">
        ${items.map(b => `
          <div class="tray-bar">
            ...
            <button class="tray-remove" onclick="removeFromCompare('${barSlug(b)}')" aria-label="Remove">×</button>
          </div>
        `).join('')}
      </div>
      <div class="tray-actions">
        <button class="tray-clear" onclick="clearCompare()">Clear</button>
        <button class="tray-compare ${items.length < 2 ? 'disabled' : ''}"
          onclick="${items.length >= 2 ? 'openCompareOverlay()' : ''}"
