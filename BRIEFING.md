...
**Remaining rev 8 candidates and which is actually next:** `low-sugar-high-protein.html`, `caffeine-protein-bars.html`, and `glp1-protein-bars.html` are still on the older structure (`picks-grid`/`verdict-card` markup, no `gd-search`/`gd-sort` filter bar) — see the "Template section order" list just below, which describes that older pattern, not rev 8. Of the three, **`low-sugar-high-protein.html` is the most overdue, not just next in list order**: its JSON-LD `dateModified` is still 2026-04-01, which predates the v5 (2026-08-12), v6 (2026-08-13), and v7 (2026-08-14) scoring pipeline fixes entirely — its published qualifying count almost certainly reflects pre-fix scores and a stale ~983-bar database, not just an old template. `caffeine-protein-bars.html` (2026-07-25) and `glp1-protein-bars.html` (2026-07-23) are stale too but by less. Recommend `low-sugar-high-protein.html` next, both for template and because its data is the most out of date of the three. Treat `no-sugar-alcohols.html`, `no-artificial-sweeteners.html`, `clean-protein-bars.html`, `best-bars-for-diabetics.html`, or `keto-protein-bars.html` as the structural/voice reference for that rebuild, not this section's section-order list, which predates rev 8 and needs a rewrite pass of its own.
