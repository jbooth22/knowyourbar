...
**Remaining rev 8 candidates:** `caffeine-protein-bars.html` and `glp1-protein-bars.html` are still on the older structure (`picks-grid`/`verdict-card` markup, no `gd-search`/`gd-sort` filter bar) — see the "Template section order" list just below, which describes that older pattern, not rev 8. `caffeine-protein-bars.html` (`dateModified` 2026-07-25) and `glp1-protein-bars.html` (2026-07-23) are both stale relative to the v5/v6/v7 scoring fixes but less severely than `low-sugar-high-protein.html` was. With that page being retired instead of rebuilt, these two are what's actually next — pick either; `caffeine-protein-bars.html` is slightly more overdue by date. Treat `no-sugar-alcohols.html`, `no-artificial-sweeteners.html`, `clean-protein-bars.html`, `best-bars-for-diabetics.html`, or `keto-protein-bars.html` as the structural/voice reference for that rebuild, not this section's section-order list, which predates rev 8 and needs a rewrite pass of its own.
...
- Guide pages all rebuilt to consistent TEMPLATE_GUIDE.html (6 of 9 — `no-seed-oils.html`, `no-sugar-alcohols.html`, `no-artificial-sweeteners.html`, `clean-protein-bars.html`, `best-bars-for-diabetics.html`, `keto-protein-bars.html` — now on the newer rev 8 standard; see "Guide pages" section above for the rest)
- Mobile hamburger menu now functional on all rev 8 guide pages (fixed 2026-08-18 for the first 4; `keto-protein-bars.html` included the same handler from the start on its 2026-08-19 rebuild, so no separate fix was needed)
