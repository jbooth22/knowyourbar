...
**Data-quality flag found during the caffeine rebuild (2026-08-20, not yet fixed):** 191 of 1,181 bars in `bars.js` have `Key: null` — including 15 of the 42 caffeinated bars (Verb, Quantum, Supervibe, G2G, Battle Bars, Jesse's WAKEUP!). Any script that uses `Key` as a unique identifier (e.g. `diff_bars_upload.py`'s `key()` function, which falls back to `Brand Name | Flavor Name` when `Key` is missing — so it's actually safe) will silently collapse every null-keyed bar into one entry if it doesn't have that same fallback. This is exactly what happened in this rebuild's first draft: macro-rank lookups collapsed all 15 null-keyed caffeinated bars onto one shared (wrong) value before being caught in QA and fixed by indexing on list position instead. Worth a dedicated session to either backfill the missing `Key` values in `bars.js` or confirm every script that reads `Key` has a safe fallback like `diff_bars_upload.py` does.
