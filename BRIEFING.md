...
**CSS:** new `.brk-*` classes appended to the end of style.css (pure append, no existing rules touched). Reuses `.bchip`, `.cert-badge`, `.boost-badge`, `.table-grade-badge`, `.macro-grid`/`.macro-card`, `.brand-compare-grade-pair`/`-arrow`, `.faq-item`, `.explore-cta*`, `.snapshot`/`.snap-item` rather than redefining equivalents.
