...
**Remaining rev 8 candidates:** `caffeine-protein-bars.html` is still on the older structure (`picks-grid`/`verdict-card` markup, no `gd-search`/`gd-sort` filter bar) — see the "Template section order" list just below, which describes that older pattern, not rev 8. `caffeine-protein-bars.html` (`dateModified` 2026-07-25) is stale relative to the v5/v6/v7 scoring fixes, same as `glp1-protein-bars.html` was before its 2026-08-19/20 rebuild. This is now the only guide page left on the old structure. Treat `no-sugar-alcohols.html`, `no-artificial-sweeteners.html`, `clean-protein-bars.html`, `keto-protein-bars.html`, or `glp1-protein-bars.html` as the structural/voice reference for that rebuild, not this section's section-order list, which predates rev 8 and needs a rewrite pass of its own.
