#!/usr/bin/env python3
"""
Build gluten-free-protein-bars.html from live bars.js.
Filter: `Gluten Free (Y/N)` = Yes (certification field, no macro/ingredient-
quality gate) -- see GUIDE_CRITERIA.md.

Reference pattern: vegan-protein-bars.html (same filter shape: a pure
certification field, not a computed macro/ingredient screen).
"""
import json, re, html as html_mod
from collections import defaultdict

REPO = '.'
OUT = './gluten-free-protein-bars.html'
...
QUALIFY = [b for b in BARS if is_yes(b.get('Gluten Free (Y/N)'))]
DISQUALIFY = [b for b in BARS if not is_yes(b.get('Gluten Free (Y/N)'))]
...
# ---------------------------------------------------------------------------
# Gluten-source category explainer (informational only, computed against the
# disqualified set -- mirrors GUIDE_CRITERIA.md's note on the Vegan guide's
# category explainer: this does not affect who qualifies, `Gluten Free (Y/N)
# = Yes` is the only qualification filter)
# ---------------------------------------------------------------------------
def ingr(b):
    return b.get('Ingredients') or ''

def matches_disq(pats):
    out = []
    for b in DISQUALIFY:
        text = ingr(b).lower()
...
WHEAT_HITS = matches_disq([r'\bwheat\b'])
BARLEY_HITS = matches_disq([r'\bbarley\b', r'\bmalt extract\b', r'\bmalt syrup\b',
                            r'\bmalted barley\b', r'\bbarley malt\b'])
UNION_KEYS = set(b['Key'] for b in WHEAT_HITS) | set(b['Key'] for b in BARLEY_HITS)
...
WHEAT_BRANDS_SHOWN, WHEAT_BRANDS_MORE = brand_split(WHEAT_HITS)
BARLEY_BRANDS_SHOWN, BARLEY_BRANDS_MORE = brand_split(BARLEY_HITS)
...
def top_level_ingredient_count(text):
    ...
    if text.count('(') != text.count(')') or text.count('[') != text.count(']'):
        return None  # malformed/truncated ingredient string -- exclude, don't fake a count
    depth, count = 0, 1
    for ch in text:
        if ch in '([':
            depth += 1
...
PICKS = [
    dict(label='Best overall', bar=best_overall,
         reason=f"{fnum(num(best_overall.get('Protein (g)')))}g protein at {fnum(num(best_overall.get('Calories')))} calories on a short, mostly whole-food list. Highest ingredient quality score of any gluten free bar in the database."),
    dict(label='Best protein/calorie ratio', bar=best_p100,
         reason=f"{fnum(num(best_p100.get('Protein (g)')))}g protein at just {fnum(num(best_p100.get('Calories')))} calories, {p100(best_p100)}g of protein per 100 calories, the most efficient split among all {QN} gluten free bars."),
    dict(label='Best total protein', bar=best_protein,
         reason=f"{fnum(num(best_protein.get('Protein (g)')))}g protein, the highest raw protein count of any gluten free bar we track."),
...
# ---------------------------------------------------------------------------
# featured worst-offender brand: recognizable brand among Wheat/Barley hits
RECOGNIZABLE = ['Quest', 'CLIF Bar', 'Barebells', 'KIND', 'RXBAR', 'Built', 'Power Crunch', 'Atkins']
featured_wheat_brand = next((br for br in RECOGNIZABLE if br in set(b['Brand Name'] for b in WHEAT_HITS)), sorted(set(b['Brand Name'] for b in WHEAT_HITS))[0])
featured_wheat_count = sum(1 for b in WHEAT_HITS if b['Brand Name'] == featured_wheat_brand)
featured_wheat_total = sum(1 for b in BARS if b['Brand Name'] == featured_wheat_brand)

print("\nFeatured wheat brand:", featured_wheat_brand, featured_wheat_count, "/", featured_wheat_total)
print("Unlabeled/no-visible-gluten disqualified:", len(UNLABELED_HITS), round(100*len(UNLABELED_HITS)/TOTAL,1), "%")
print("Wheat disqualified:", len(WHEAT_HITS), round(100*len(WHEAT_HITS)/TOTAL,1), "%")
print("Barley disqualified:", len(BARLEY_HITS), round(100*len(BARLEY_HITS)/TOTAL,1), "%")
...
WHEAT_KEYS = set(b['Key'] for b in WHEAT_HITS)
BARLEY_KEYS = set(b['Key'] for b in BARLEY_HITS)
...
def culprit_for(disq_bars):
    has_wheat = any(b['Key'] in WHEAT_KEYS for b in disq_bars)
    has_barley = any(b['Key'] in BARLEY_KEYS for b in disq_bars)
    if has_wheat and has_barley:
        return 'Wheat and barley malt'
    if has_wheat:
        return 'Wheat'
    if has_barley:
        return 'Barley malt'
    return 'Not labeled gluten free'
...
RANK_METRICS = [
    ...
    ('ft', 'Total Fat (g)', None),  # neutral, always gray
]
...
# ======================================================================
# assemble.py content (page assembly)
# ======================================================================
...
# ---------------------------------------------------------------------------
TITLE = f"{PCT_QUALIFY:.0f}% of Protein Bars Are Gluten Free. See All {QN}."
H1 = f"Best Gluten Free Protein Bars - Ranking {N_ROUND}+ Qualified Bars"
META_DESC = f"We checked {TOTAL:,} protein bars against their gluten free label. {QN} qualify. See every one, ranked by ingredient quality, brand, and macros."
OG_DESC = f"{QN} gluten free protein bars with no wheat or barley in the recipe. Ranked by ingredient quality score."
HERO_SUB = (f"We checked {TOTAL:,} protein bars available in the US against their Gluten Free (Y/N) label. "
            f"The result: {QN} bars, about {PCT_QUALIFY:.0f}%, are labeled gluten free. "
            f"We rank the best gluten free protein bars by ingredient quality, brand, and macros. Not just us telling you the flavors we like.")

# ---------------------------------------------------------------------------
# Top picks HTML + ItemList JSON-LD data
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Category explainer (Wheat / Barley-Malt / Not Labeled)
# ---------------------------------------------------------------------------
...
WHEAT_PCT = round(100 * len(WHEAT_HITS) / TOTAL, 1)
BARLEY_PCT = round(100 * len(BARLEY_HITS) / TOTAL, 1)
UNLABELED_PCT = round(100 * len(UNLABELED_HITS) / TOTAL, 1)

FACTOR_WHEAT = factor_card_html(
    'Wheat', len(WHEAT_HITS), WHEAT_PCT,
    'The most common named gluten source, usually as wheat flour, a wheat-based crisp, or a graham/cookie piece mixed into the bar.',
    WHEAT_BRANDS_SHOWN, WHEAT_BRANDS_MORE)
FACTOR_BARLEY = factor_card_html(
    'Barley / Malt', len(BARLEY_HITS), BARLEY_PCT,
    'Shows up as barley malt extract or malt syrup, usually a flavoring or sweetener rather than a structural ingredient.',
    BARLEY_BRANDS_SHOWN, BARLEY_BRANDS_MORE)
FACTOR_UNLABELED = f'''<div class="score-card">
  <div class="score-card-label">Not labeled gluten free</div>
  <div class="score-card-val">{len(UNLABELED_HITS)} bars<span class="oil-card-pct">{UNLABELED_PCT}%</span></div>
  <div class="score-card-desc">No wheat or barley shows up anywhere in the ingredient list we can find, but the brand hasn't labeled or certified the bar gluten free either. That is a labeling gap, not proof the bar contains gluten.</div>
</div>'''

CATEGORY_EXPLAINER_P1 = (f"We check the Gluten Free (Y/N) label on file for every bar, then cross-check ingredient lists ourselves for wheat and barley or malt, "
                         f"the two gluten sources that show up by name in our data. {QN} of {TOTAL:,} bars, about {PCT_QUALIFY:.0f}%, carry a gluten free label. "
                         f"The other {DN}, about {PCT_DISQUALIFY:.0f}%, don't.")
CATEGORY_EXPLAINER_P2 = (f"Wheat is the most common named culprit at {len(WHEAT_HITS)} bars ({WHEAT_PCT}% of the full database), "
                         f"followed by barley or malt at {len(BARLEY_HITS)} bars ({BARLEY_PCT}%). "
                         f"The remaining {len(UNLABELED_HITS)} bars, {UNLABELED_PCT}% of the full database, show no wheat or barley in their own ingredient list at all. "
                         f"They just aren't labeled gluten free, which is a different claim from actually containing gluten.")

# ---------------------------------------------------------------------------
# Findings section
# ---------------------------------------------------------------------------
fully_gf_brands = [r for r in CONSIDER if r['q'] == r['total']]
fully_gf_top = sorted(fully_gf_brands, key=lambda r: -r['total'])[:4]
fully_gf_names = ', '.join(f"{r['brand']} ({r['total']})" for r in fully_gf_top)

nugo = next((r for r in MIXED if r['brand'] == 'NuGo'), None)

BIG_STAT_NUM = f"{UNLABELED_PCT}%"
BIG_STAT_HEAD = "of all protein bars show no wheat or barley, yet still aren't labeled gluten free"
BIG_STAT_DETAIL = (f"{len(UNLABELED_HITS)} of the {TOTAL:,} bars we track have no wheat or barley anywhere in their own ingredient list, "
                   f"but the brand hasn't labeled or certified them gluten free. Not being labeled gluten free is not the same as containing gluten.")

INSIGHTS = [
    ("Wheat is the clearest single gluten source.",
     f"{len(WHEAT_HITS)} bars ({WHEAT_PCT}% of the database) name wheat directly, more than double the barley/malt count."),
    (f"{featured_wheat_brand} shows the pattern clearly.",
     f"{featured_wheat_count} of {featured_wheat_brand}'s {featured_wheat_total} flavors use wheat directly. None of {featured_wheat_brand}'s flavors are labeled gluten free."),
    (f"{len(fully_gf_brands)} brands are gluten free across their entire lineup.",
     f"Led by {fully_gf_names}, these brands never reach for wheat or barley in the first place, rather than swapping it out flavor by flavor."),
    ("Gluten free bars grade slightly better than the database average.",
     f"{AB_RATE_Q}% of gluten free bars grade A or B, against {AB_RATE_ALL}% database-wide."),
    ("Gluten free bars average less protein, about the same fiber.",
     f"Gluten free bars average {fnum(AVG_PROTEIN_Q)}g of protein against a database-wide average of {fnum(AVG_PROTEIN_ALL)}g, but carry about the same fiber ({fnum(AVG_FIBER_Q)}g vs. {fnum(AVG_FIBER_ALL)}g)."),
    (f"{nugo['brand']} splits closer to even than any other brand." if nugo else "Mixed lineups are rare.",
     (f"{nugo['q']} of {nugo['brand']}'s {nugo['total']} flavors are gluten free, the rest are not. It is the largest genuinely mixed lineup in the database."
      if nugo else "Most brands are either fully gluten free or not gluten free at all; very few split their lineup.")),
]
...
def consider_row(r):
    return (f'<tr><td><button type="button" class="brand-jump" data-brand="{esc(r["brand"])}">{esc(r["brand"])}</button></td>'
            ...
            f'<td>{fnum(r["avg_p"])}g</td><td>{fnum(r["avg_s"])}g</td><td>Gluten free across the whole lineup</td></tr>')
...
def mixed_row(r):
    return (f'<tr><td><button type="button" class="brand-jump" data-brand="{esc(r["brand"])}">{esc(r["brand"])}</button></td>'
            ...
            f'<td>{fnum(r["avg_p"])}g</td><td>{fnum(r["avg_s"])}g</td><td>{esc(r["clean_pick"])} is the gluten free pick</td></tr>')
...
AVOID_SHOW_MORE = ''
if AVOID_HIDDEN:
    AVOID_SHOW_MORE = f'''<button type="button" class="brand-table-show-more" id="gf-avoid-show-more" data-hidden-count="{len(AVOID_HIDDEN)}">Show {len(AVOID_HIDDEN)} more brands</button>
<script>
(function() {{
  var btn = document.getElementById('gf-avoid-show-more');
  var table = document.getElementById('gf-avoid-table');
  if (!btn || !table) return;
...
# ---------------------------------------------------------------------------
# Bar table: sort by ingredient_score desc, first 60 rich, rest lazy
# ---------------------------------------------------------------------------
...
# broken link-field scan (QA.md standing rule) before anything else touches this data
...
# ---------------------------------------------------------------------------
# FAQ (11 questions, must match JSON-LD exactly)
# ---------------------------------------------------------------------------
FAQ = [
    ("What makes a protein bar gluten free on this site?",
     f"We use the Gluten Free (Y/N) label on file for each bar. {QN} of the {TOTAL:,} bars we track carry that label. We also cross-check ingredient lists ourselves for wheat and barley or malt, the two named gluten sources that show up most in the data."),
    ("How many gluten free protein bars are in your database?",
     f"{QN} of the {TOTAL:,} bars we track are labeled gluten free, spanning {len(BRANDS_Q)} brands. {A_GRADE_Q} of those {QN} bars grade A for ingredient quality."),
    (f"Is {featured_wheat_brand} gluten free?",
     f"No. {featured_wheat_count} of {featured_wheat_brand}'s {featured_wheat_total} flavors use wheat directly, and none of {featured_wheat_brand}'s flavors carry a gluten free label."),
    ("Are Larabar bars gluten free?",
     "Yes. All 29 Larabar flavors we track are labeled gluten free. Larabar builds its bars around dates, nuts, and fruit rather than a wheat-based binder or crisp."),
    ("What is the most common gluten ingredient in protein bars?",
     f"Wheat. It shows up by name in {len(WHEAT_HITS)} of the {TOTAL:,} bars we track, more than double the count for barley or malt. Most other non-gluten-free bars simply aren't labeled, without a named gluten ingredient we can find."),
    ("Does not being labeled gluten free mean a bar contains gluten?",
     f"Not necessarily. {len(UNLABELED_HITS)} of the {DN} bars that don't carry our gluten free label show no wheat or barley anywhere in their own ingredient list. The brand just hasn't labeled or certified the bar, which is a different claim from it containing gluten."),
    ("Can a brand have some gluten free flavors and some that aren't?",
     (f"Yes. {nugo['brand']} splits closest to even of any brand we checked: {nugo['q']} of {nugo['total']} flavors are gluten free. Always check the specific flavor, not just the brand."
    ...
    ("What protein bars are gluten free?",
     f"{QN} bars across {len(BRANDS_Q)} brands carry a gluten free label, led by brands like Larabar and think! that qualify across their entire lineup. The full ranked list is in the table below, sorted by ingredient quality."),
    ("What protein bars are not gluten free?",
     f"{DN} of the {TOTAL:,} bars we track don't carry a gluten free label. Wheat is the most common named reason, followed by barley or malt. Most of the remaining bars simply haven't been labeled, without an identifiable gluten ingredient in the list."),
    ("Are gluten free protein bars lower quality than regular bars?",
     f"No. Gluten free bars in our database grade A or B at a slightly higher rate than the database as a whole ({AB_RATE_Q}% vs. {AB_RATE_ALL}%). What they give up on average is protein: gluten free bars average {fnum(AVG_PROTEIN_Q)}g against a database-wide average of {fnum(AVG_PROTEIN_ALL)}g."),
...
# ---------------------------------------------------------------------------
# Nav / footer -- reused verbatim from vegan-protein-bars.html, with this
# guide's own link appended (own-nav self-listing, matches existing pattern)
# ---------------------------------------------------------------------------
with open(f'{REPO}/vegan-protein-bars.html') as f:
    VEGAN_SRC = f.read()
...
NAV_BLOCK = extract(VEGAN_SRC, '<nav class="site-nav">', '</nav>') + '</nav>'
...
NAV_TOGGLE_SCRIPT = extract(VEGAN_SRC, "<script>\n(function() {\n  var navToggle", "})();\n</script>") + "})();\n</script>"

FOOTER_BLOCK = extract(VEGAN_SRC, '<footer class="site-footer">', '</footer>') + '</footer>'
FOOTER_BLOCK = FOOTER_BLOCK.replace(
    '<a href="/vegan-protein-bars.html">Vegan Protein Bars</a>\n    </nav>',
    '<a href="/vegan-protein-bars.html">Vegan Protein Bars</a>\n      <a href="/gluten-free-protein-bars.html">Gluten Free Protein Bars</a>\n    </nav>'
)
FOOTER_BLOCK = re.sub(r'Updated [A-Za-z]+ \d{4}', 'Updated August 2026', FOOTER_BLOCK)

# one big final <script> block: filter/sort IIFE + toggleIngr + FAQ accordion
# + buildLazyExpand -- fully generic, copy verbatim per TEMPLATE_GUIDE.html
MAIN_SCRIPT = extract(VEGAN_SRC, '<script>\n(function() {\n  var tbody', '\n</script>\n\n</body>') + '\n</script>'

assert 'gluten-free-protein-bars.html' in NAV_BLOCK
assert 'gluten-free-protein-bars.html' in FOOTER_BLOCK
...
# ---------------------------------------------------------------------------
# Snapshot metric 3/4/5 (avg protein, brands represented, A-grade count already used elsewhere)
# ---------------------------------------------------------------------------
SNAPSHOT_METRICS = [
...
<head>
  ...
  {{
    ...
    "about": {{ "@type": "Thing", "name": "Gluten Free Protein Bars" }}
  }}
  </script>
  ...
  {{
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": "Top picks for gluten free protein bars",
    "itemListElement": [
...
<main class="content">
  ...
  <section class="section">
    <div class="section-inner">
      <h2 class="section-title">Top picks for gluten free protein bars</h2>
      <p class="section-body">Everyone has their own reason for wanting a protein bar, but if you're on this page, you already know you want one that's gluten free. Here are the best bars for what people typically look for, all gluten free. Grades below reflect ingredient quality only, not an overall bar rating.</p>
...
{INSIGHTS_HTML}
  ...
  <section class="section">
    <div class="section-inner">
      <h2 class="section-title">Best Brands of Gluten Free Protein Bars</h2>
      <div class="section-body">
        <p>Some brands build their whole lineup without wheat or barley, others lean on it across the board. Grade columns below show ingredient quality only, not an overall bar rating. Click any brand name to jump to its flavors in the table below.</p>
      </div>

      <div class="brand-table-block">
        <div class="brand-table-label pro">Brands to Consider</div>
        <div class="brand-table-note">Every flavor from these {len(CONSIDER)} brands is gluten free.</div>
...
{CONSIDER_ROWS}
      ...
      <div class="brand-table-block">
        ...
        <div class="brand-table-note">These brands lean on wheat, barley, or an unlabeled formula across most or all of their lineup.</div>
        <div class="table-scroll">
          <table class="brand-table" id="gf-avoid-table">
            <thead><tr><th>Brand</th><th>Flavors Without Gluten Free Label</th><th>Ingredient Quality</th><th>Avg Protein</th><th>Avg Sugar</th><th>Gluten Source Found</th></tr></thead>
            <tbody>
{AVOID_ROWS}
            </tbody>
          </table>
      ...
      <div class="brand-table-block">
        ...
        <div class="brand-table-note">Some flavors are gluten free, some aren't. Check the specific flavor before buying.</div>
        <div class="table-scroll">
          <table class="brand-table">
            <thead><tr><th>Brand</th><th>Gluten Free Flavors</th><th>Ingredient Quality</th><th>Avg Protein</th><th>Avg Sugar</th><th>Gluten Free Pick</th></tr></thead>
            <tbody>
{MIXED_ROWS}
            </tbody>
          </table>
  ...
  <section class="bars-section">
    <div class="bars-section-inner">
      <h2 class="section-title">{QN} gluten free protein bars, ranked by ingredient quality</h2>
      <p class="section-body">Every gluten free bar in the database, sorted best to worst by ingredient quality grade. Use the sort dropdown to re-rank by protein, calories, or sugar instead, or filter by grade. Tap any row for the full macro rank, ingredient breakdown, and buy links.</p>
...
{BAR_ROWS_HTML}
      ...
      <div class="show-more-wrap" id="show-more-wrap">
        <button class="show-more-btn" id="gd-show-more">Show more bars</button>
      </div>
  ...
  <section class="section guide-finder-cta">
    <div class="explore-cta-grid">
      <div class="explore-cta-main">
        ...
        <p class="explore-cta-main-desc">This guide covers gluten free bars specifically. The Bar Finder covers all 1,000+ bars in the database, and you set the filters: protein, sugar, fiber, calories, ingredient grade, brand, certifications, and specific ingredients to exclude. No sponsored picks, no subjective taste tests. You decide what matters.</p>
        <a href="/bar-finder.html?certs=GF" class="finder-cta-btn">Open the Bar Finder &rarr;</a>
      </div>
    ...
    <div class="section-inner">
      ...
      <div class="explore-more-grid">
        <a href="/vegan-protein-bars.html" class="explore-more-card">
          ...
          <div class="explore-more-desc">255 bars with no whey, milk, honey, egg, or gelatin.</div>
        </a>
        <a href="/no-seed-oils.html" class="explore-more-card">
