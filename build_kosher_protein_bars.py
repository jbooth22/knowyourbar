#!/usr/bin/env python3
"""
Build kosher-protein-bars.html from live bars.js.
Filter: `Kosher (Y/N)` = Yes (certification field, no macro/ingredient-
...
QUALIFY = [b for b in BARS if is_yes(b.get('Kosher (Y/N)'))]
DISQUALIFY = [b for b in BARS if not is_yes(b.get('Kosher (Y/N)'))]
...
# ---------------------------------------------------------------------------
# Kosher category explainer (informational only, computed against the
# disqualified set -- mirrors GUIDE_CRITERIA.md's note on the Vegan/Gluten
# Free/Dairy Free/Soy Free guides' category explainers: this does not affect
# who qualifies, `Kosher (Y/N) = Yes` is the only qualification filter).
#
# Kosher is structurally different from Dairy Free/Soy Free/Gluten Free: it's
# a supervised-process certification, not primarily an ingredient screen. Most
# ingredients in a protein bar (whey, milk, soy, wheat, sugar, nuts) can all
# be kosher when produced and supervised correctly -- an ingredient's mere
# presence doesn't disqualify a bar the way whey disqualifies Dairy Free.
# Three named ingredients are the exception, ingredients that are almost
# never kosher without their own specific certification: Gelatin (usually
# derived from pork or non-ritually-slaughtered animal sources), Confectioner's
# Glaze / Shellac (a resin secreted by the lac insect, used as a shiny coating
# on some candy-coated pieces), and a small combined Other bucket for Carmine/
# Cochineal (an insect-derived red colorant) and Rennet (traditionally
# animal-derived, used in some dairy-protein blends). Checked and confirmed
# no overlap issue: these three checks are run independently against the
# disqualified set and a bar can in principle hit more than one.
# ---------------------------------------------------------------------------
def ingr(b):
    return b.get('Ingredients') or ''

SOY_PROTEIN_PAT = re.compile(r'\bgelatin\b')
SOY_LECITHIN_PAT = re.compile(r'shellac|confectioner.?s glaze')
SOYBEAN_OIL_PAT = re.compile(r'\bcarmine\b|cochineal|\brennet\b')
...
PICKS = [
    dict(label='Best overall', bar=best_overall,
         reason=f"{fnum(num(best_overall.get('Protein (g)')))}g protein at {fnum(num(best_overall.get('Calories')))} calories on a short, mostly whole-food list. Highest ingredient quality score of any kosher bar in the database."),
    dict(label='Best protein/calorie ratio', bar=best_p100,
         reason=f"{fnum(num(best_p100.get('Protein (g)')))}g protein at just {fnum(num(best_p100.get('Calories')))} calories, {p100(best_p100)}g of protein per 100 calories, the most efficient split among all {QN} kosher bars."),
    dict(label='Best total protein', bar=best_protein,
         reason=f"{fnum(num(best_protein.get('Protein (g)')))}g protein, the highest raw protein count of any kosher bar we track. Kosher bars average noticeably less protein than the database as a whole, so a kosher bar hitting this number is worth knowing about."),
...
print("\nFeatured gelatin brand:", featured_whey_brand, featured_whey_count, "/", featured_whey_total)
print("Gelatin:", len(WHEY_HITS), round(100*len(WHEY_HITS)/TOTAL, 1), "%")
print("Confectioner's glaze/shellac:", len(MILK_HITS), round(100*len(MILK_HITS)/TOTAL, 1), "%")
print("Other (carmine/rennet):", len(CASEIN_HITS), round(100*len(CASEIN_HITS)/TOTAL, 1), "%")
print("Unlabeled/no-visible-animal-derived disqualified:", len(UNLABELED_HITS), round(100*len(UNLABELED_HITS)/TOTAL, 1), "%")
...
def culprit_for(disq_bars):
    ...
    parts = []
    if has_whey:
        parts.append('Gelatin')
    if has_milk:
        parts.append("Confectioner's Glaze")
    if has_casein:
        parts.append('Other Animal-Derived')
    if parts:
        return ' and '.join(parts)
    return 'Not kosher-certified'
...
# ---------------------------------------------------------------------------
TITLE = f"{PCT_QUALIFY:.0f}% of Protein Bars Are Kosher. See All {QN}."
H1 = f"Best Kosher Protein Bars - Ranking {N_ROUND}+ Qualified Bars"
META_DESC = f"We checked {TOTAL:,} protein bars against their kosher label. {QN} qualify. See every one, ranked by ingredient quality, brand, and macros."
OG_DESC = f"{QN} kosher-certified protein bars. Ranked by ingredient quality score."
HERO_SUB = (f"We checked {TOTAL:,} protein bars available in the US against their Kosher (Y/N) label. "
            f"The result: {QN} bars, about {PCT_QUALIFY:.0f}%, are labeled kosher. Most of the rest simply haven't sought certification, not because of a specific non-kosher ingredient. "
            f"We rank the best kosher protein bars by ingredient quality, brand, and macros. Not just us telling you the flavors we like.")
...
# DN-relative versions (share of the DISQUALIFIED/non-kosher set specifically,
# not the full database) -- used where copy explicitly says "of non-kosher
# bars" rather than "of the database", so the percentages stated actually
# match the denominator named in the sentence.
WHEY_PCT_OF_DN = round(100 * len(WHEY_HITS) / DN, 1)
MILK_PCT_OF_DN = round(100 * len(MILK_HITS) / DN, 1)
CASEIN_PCT_OF_DN = round(100 * len(CASEIN_HITS) / DN, 1)
UNLABELED_PCT_OF_DN = round(100 * len(UNLABELED_HITS) / DN, 1)

FACTOR_WHEY = factor_card_html(
    'Gelatin', len(WHEY_HITS), WHEY_PCT,
    'Almost always derived from pork or non-ritually-slaughtered animal collagen, used in the puff or crisp layer of several bars. Fish and certified-kosher beef gelatin exist, but plain "gelatin" on a US ingredient label defaults to non-kosher.',
    WHEY_BRANDS_SHOWN, WHEY_BRANDS_MORE)
FACTOR_MILK = factor_card_html(
    "Confectioner's Glaze / Shellac", len(MILK_HITS), MILK_PCT,
    'A resin secreted by the lac insect, used as a shiny coating on some candy-coated pieces and drizzles. An insect-derived ingredient, not kosher without its own certification.',
    MILK_BRANDS_SHOWN, MILK_BRANDS_MORE)
FACTOR_CASEIN = factor_card_html(
    'Other Animal-Derived (Carmine, Rennet)', len(CASEIN_HITS), CASEIN_PCT,
    'Carmine/cochineal (an insect-derived red colorant) and rennet (traditionally animal-derived, used in some dairy-protein blends). The rarest of the three named factors here.',
    CASEIN_BRANDS_SHOWN, CASEIN_BRANDS_MORE)
FACTOR_UNLABELED = f'''<div class="score-card">
  <div class="score-card-label">Not kosher-certified</div>
  <div class="score-card-val">{len(UNLABELED_HITS)} bars<span class="oil-card-pct">{UNLABELED_PCT}%</span></div>
  <div class="score-card-desc">No gelatin, confectioner's glaze, carmine, or rennet shows up anywhere in the ingredient list we can find. These bars aren't disqualified by a specific non-kosher ingredient, the brand simply hasn't pursued kosher certification, which is a supervised-process claim, not just an ingredient checklist.</div>
</div>'''

CATEGORY_EXPLAINER_P1 = (f"We check the Kosher (Y/N) label on file for every bar. Kosher is different from Vegan, Gluten Free, Dairy Free, and Soy Free: it's a supervised-process certification, "
                         f"not primarily an ingredient screen. Whey, milk, soy, wheat, sugar, and nuts can all be kosher when produced and supervised correctly, so an ingredient's mere presence usually doesn't disqualify a bar the way whey disqualifies Dairy Free. "
                         f"{QN} of {TOTAL:,} bars, about {PCT_QUALIFY:.0f}%, carry a kosher label. The other {DN}, about {PCT_DISQUALIFY:.0f}%, don't.")
CATEGORY_EXPLAINER_P2 = (f"We do cross-check ingredient lists for three ingredients that are almost never kosher without their own specific certification: gelatin ({len(WHEY_HITS)} bars, {WHEY_PCT}%), "
                         f"confectioner's glaze or shellac ({len(MILK_HITS)} bars, {MILK_PCT}%), and carmine or rennet ({len(CASEIN_HITS)} bars, {CASEIN_PCT}%). "
                         f"Together those account for only a small share of the {DN} bars that aren't labeled kosher. The remaining {len(UNLABELED_HITS)} bars, {UNLABELED_PCT}% of the full database, "
                         f"show none of those ingredients at all. They simply haven't been through kosher certification, which is a different claim from containing something non-kosher.")
...
# Unlike Dairy Free/Soy Free, where a majority of disqualified bars have an
# identifiable disqualifying ingredient, kosher disqualification is almost
# entirely a certification gap, not an ingredient gap. Confirmed against the
# 2026-09-17 build: only ~7.6% of disqualified bars contain gelatin,
# confectioner's glaze, carmine, or rennet, the rest simply aren't certified.
def has_tag(b, tag):
...
BIG_STAT_NUM = f"{UNLABELED_PCT_OF_DN}%"
BIG_STAT_HEAD = "of non-kosher protein bars have no identifiable non-kosher ingredient at all"
BIG_STAT_DETAIL = (f"{len(UNLABELED_HITS)} of the {DN} bars without a kosher label show no gelatin, confectioner's glaze, carmine, or rennet anywhere in their own ingredient list. "
                   f"For most brands, not being kosher-certified is a business decision, not a formulation problem. Certification requires an ongoing supervised process, and plenty of brands simply haven't pursued it.")

INSIGHTS = [
    ("Kosher disqualification is almost always about certification, not ingredients.",
     f"Only {WHEY_PCT_OF_DN + MILK_PCT_OF_DN + CASEIN_PCT_OF_DN:.1f}% of non-kosher bars combined contain gelatin, confectioner's glaze, carmine, or rennet, the three named factors we can actually detect. The other {UNLABELED_PCT_OF_DN}% simply haven't sought certification."),
    (f"{featured_whey_brand} shows the gelatin pattern clearly.",
     f"{featured_whey_count} of {featured_whey_brand}'s {featured_whey_total} flavors use gelatin directly, in the puff or crisp layer. None of {featured_whey_brand}'s flavors are labeled kosher."),
    (f"{len(fully_df_brands)} brands are kosher across their entire lineup.",
     f"Led by {fully_df_names}, these brands have pursued kosher certification across the board rather than for a single flagship flavor."),
    ("Kosher bars grade about the same as the database average, slightly below.",
     f"{AB_RATE_Q}% of kosher bars grade A or B, against {AB_RATE_ALL}% database-wide. Kosher certification doesn't track with ingredient quality either way, it's a separate axis entirely."),
    ("Kosher bars average notably less protein and less fiber.",
     f"Kosher bars average {fnum(AVG_PROTEIN_Q)}g of protein against a database-wide average of {fnum(AVG_PROTEIN_ALL)}g, and {fnum(AVG_FIBER_Q)}g of fiber against {fnum(AVG_FIBER_ALL)}g database-wide. The kosher-certified set skews toward smaller, simpler snack-style bars rather than high-protein performance bars."),
    ((f"{nugo['brand']} splits closer to even than most brands." if nugo else "Mixed lineups are rare."),
     (f"{nugo['q']} of {nugo['brand']}'s {nugo['total']} flavors are kosher, the rest are not. Always check the specific flavor, not just the brand."
      if nugo else "Most brands are either fully kosher or not kosher at all; very few split their lineup.")),
]
...
def consider_row(r):
    return (f'<tr><td><button type="button" class="brand-jump" data-brand="{esc(r["brand"])}">{esc(r["brand"])}</button></td>'
            ...
            f'<td>{fnum(r["avg_p"])}g</td><td>{fnum(r["avg_s"])}g</td><td>Kosher across the whole lineup</td></tr>')
...
def mixed_row(r):
    return (f'<tr><td><button type="button" class="brand-jump" data-brand="{esc(r["brand"])}">{esc(r["brand"])}</button></td>'
            ...
            f'<td>{fnum(r["avg_p"])}g</td><td>{fnum(r["avg_s"])}g</td><td>{esc(r["clean_pick"])} is the kosher pick</td></tr>')
...
AVOID_SHOW_MORE = ''
if AVOID_HIDDEN:
    AVOID_SHOW_MORE = f'''<button type="button" class="brand-table-show-more" id="ks-avoid-show-more" data-hidden-count="{len(AVOID_HIDDEN)}">Show {len(AVOID_HIDDEN)} more brands</button>
<script>
(function() {{
  var btn = document.getElementById('ks-avoid-show-more');
  var table = document.getElementById('ks-avoid-table');
  if (!btn || !table) return;
...
FAQ = [
    ("What makes a protein bar kosher on this site?",
     f"We use the Kosher (Y/N) label on file for each bar. {QN} of the {TOTAL:,} bars we track carry that label. Kosher is a supervised-process certification, not just an ingredient list, so we can't fully verify it ourselves the way we can with an ingredient-based screen. We do flag three ingredients that are almost never kosher without their own certification: gelatin, confectioner's glaze or shellac, and carmine or rennet."),
    ("How many kosher protein bars are in your database?",
     f"{QN} of the {TOTAL:,} bars we track are labeled kosher, spanning {len(BRANDS_Q)} brands. {A_GRADE_Q} of those {QN} bars grade A for ingredient quality."),
    (f"Is {featured_whey_brand} kosher?",
     f"No. {featured_whey_count} of {featured_whey_brand}'s {featured_whey_total} flavors use gelatin directly, and none of {featured_whey_brand}'s flavors carry a kosher label."),
    ("What ingredient most often disqualifies a bar from being kosher?",
     f"Gelatin. It shows up by name in {len(WHEY_HITS)} of the {TOTAL:,} bars we track, more than confectioner's glaze, carmine, and rennet combined, usually in a puff or crisp layer. But it's the exception, not the rule: most non-kosher bars don't contain any of the ingredients we flag at all."),
    ("Does not being labeled kosher mean a bar contains a non-kosher ingredient?",
     f"Not necessarily, and usually not. {len(UNLABELED_HITS)} of the {DN} bars that don't carry our kosher label, {UNLABELED_PCT_OF_DN}%, show no gelatin, confectioner's glaze, carmine, or rennet anywhere in their own ingredient list. For most brands, not being kosher-certified just means they haven't pursued the certification, not that the bar contains something non-kosher."),
    ("Can a brand have some kosher flavors and some that aren't?",
     (f"Yes. {nugo['brand']} is a good example: {nugo['q']} of {nugo['total']} flavors are kosher, the rest are not. Always check the specific flavor, not just the brand."
    ...
    ("What protein bars are kosher?",
     f"{QN} bars across {len(BRANDS_Q)} brands carry a kosher label. The full ranked list is in the table below, sorted by ingredient quality."),
    ("What protein bars are not kosher?",
     f"{DN} of the {TOTAL:,} bars we track don't carry a kosher label. Most simply haven't gone through kosher certification. Gelatin is the most common of the few ingredients we can actually flag as a likely reason, followed by confectioner's glaze and carmine or rennet."),
    ("Are kosher protein bars lower or higher quality than regular bars?",
     f"About the same. Kosher bars in our database grade A or B {AB_RATE_Q}% of the time, close to the {AB_RATE_ALL}% database-wide rate. Kosher certification is a separate claim from ingredient quality; it doesn't track with a higher or lower grade either way. What kosher bars do give up on average is protein and fiber: they average {fnum(AVG_PROTEIN_Q)}g of protein and {fnum(AVG_FIBER_Q)}g of fiber, against database-wide averages of {fnum(AVG_PROTEIN_ALL)}g and {fnum(AVG_FIBER_ALL)}g. The kosher-certified set skews toward smaller, simpler snack-style bars."),
    ("Is kosher the same as vegan?",
     "No. A kosher bar can still contain whey, milk, eggs, or honey, none of which are vegan. Kosher also allows kosher-slaughtered meat and poultry, which obviously isn't vegan either. Check our Vegan Protein Bars guide separately if that's what you need."),
    ("How often is this list updated?",
     f"We update the database whenever new bars are added or a brand reformulates. Certifications can also lapse or get added, so always confirm against the packaging or the brand's own kosher documentation. This page reflects the database as of {DATE_MODIFIED}."),
]
...
# shipped guide, carries the full current 15-guide nav/footer link set),
# with this guide's own link appended after Soy Free (the last guide link
# in that set).
# ---------------------------------------------------------------------------
with open(f'{REPO}/soy-free-protein-bars.html') as f:
    GF_SRC = f.read()
...
NAV_BLOCK = NAV_BLOCK.replace(
    ...
    '<a href="/soy-free-protein-bars">Soy Free Protein Bars</a>\n        <a href="/kosher-protein-bars">Kosher Protein Bars</a>\n      </div>'
)
...
assert 'kosher-protein-bars' in NAV_BLOCK
assert 'kosher-protein-bars' in FOOTER_BLOCK
...
(function(){{
  ...
  {{
    ...
    "about": {{ "@type": "Thing", "name": "Kosher Protein Bars" }}
  }}
  </script>
  ...
  {{
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": "Top picks for kosher protein bars",
    "itemListElement": [
...
<main class="content">
  ...
  <section class="section">
    <div class="section-inner">
      <h2 class="section-title">Top picks for kosher protein bars</h2>
      <p class="section-body">Everyone has their own reason for wanting a protein bar, but if you're on this page, you already know you want one that's kosher. Here are the best bars for what people typically look for, all kosher. Grades below reflect ingredient quality only, not an overall bar rating.</p>
...
{PICKS_HTML}
  ...
  <section class="section off">
    <div class="section-inner">
      <h2 class="section-title">What disqualifies a protein bar from being kosher</h2>
...
{INSIGHTS_HTML}
  ...
  <section class="section">
    <div class="section-inner">
      <h2 class="section-title">Best Brands of Kosher Protein Bars</h2>
      <div class="section-body">
        <p>Some brands pursue kosher certification across their whole lineup, others don't have a single kosher flavor. Grade columns below show ingredient quality only, not an overall bar rating. Click any brand name to jump to its flavors in the table below.</p>
      </div>

      <div class="brand-table-block">
        <div class="brand-table-label pro">Brands to Consider</div>
        <div class="brand-table-note">Every flavor from these {len(CONSIDER)} brands is kosher.</div>
...
{CONSIDER_ROWS}
      ...
      <div class="brand-table-block">
        ...
        <div class="brand-table-note">These brands aren't kosher-certified across most or all of their lineup, whether or not a specific non-kosher ingredient is identifiable.</div>
        <div class="table-scroll">
          <table class="brand-table" id="ks-avoid-table">
            <thead><tr><th>Brand</th><th>Flavors Without Kosher Label</th><th>Ingredient Quality</th><th>Avg Protein</th><th>Avg Sugar</th><th>Likely Reason</th></tr></thead>
            <tbody>
{AVOID_ROWS}
            </tbody>
          </table>
      ...
      <div class="brand-table-block">
        ...
        <div class="brand-table-note">Some flavors are kosher, some aren't. Check the specific flavor before buying.</div>
        <div class="table-scroll">
          <table class="brand-table">
            <thead><tr><th>Brand</th><th>Kosher Flavors</th><th>Ingredient Quality</th><th>Avg Protein</th><th>Avg Sugar</th><th>Kosher Pick</th></tr></thead>
            <tbody>
{MIXED_ROWS}
            </tbody>
          </table>
  ...
  <section class="bars-section">
    <div class="bars-section-inner">
      <h2 class="section-title">{QN} kosher protein bars, ranked by ingredient quality</h2>
      <p class="section-body">Every kosher bar in the database, sorted best to worst by ingredient quality grade. Use the sort dropdown to re-rank by protein, calories, or sugar instead, or filter by grade. Tap any row for the full macro rank, ingredient breakdown, and buy links.</p>
